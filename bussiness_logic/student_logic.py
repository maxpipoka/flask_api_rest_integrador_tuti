...
class StudentLogic:
    ...
    def get_studentes(self) -> list[Student]:
        ...
        active_students = Student.query.filter(Student.active == True).order_by(
            Student.id
        )
    ...
    def associate_tutor_with_student(
        self, student_id: int, tutor_id: int
    ) -> dict[str, any]:
        """
        ...
        if tutor_id in student.tutors:
            raise ValueError(
                f"Tutor with ID {tutor_id} is already associated with student {student_id}"
            )
